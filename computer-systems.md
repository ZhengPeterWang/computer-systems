...
在汇编语言实现中，函数调用是值得特别注意的。为了让编译器正确编译，我们必须规定一套调用惯例（即寄存器使用惯例）。尽管RISC-V寄存器文件中32个寄存器都是通用寄存器，我们一般规定每个寄存器都有不同的用途。下表列出了RISC-V32个寄存器的调用惯例。

寄存器 | 别名 | 用途 | 备注
-|-|-|-
x0 | - | 返回0值 | 永远是0，不可写入
x1 | ra | 在函数调用正在进行时，存储返回时程序应跳到的地址 | 由调用者负责存储
x2 | sp | 在函数调用时，存储内存中调用栈的第一个地址 | 由正在执行的函数负责存储。注意：调用栈的第一个地址是整个调用栈的最下端
x3 | fp | 在函数调用时，存储内存中属于本函数调用栈的最后一个地址 | 由正在执行的函数负责存储。注意：调用栈最后一个地址是整个调用栈的最上端

把返回值在内存中的地址ra存在寄存器x1中；在寄存器sp（x2）中存着内存中调用栈指针（地址），方便递归；一般来说，函数的前8个参数存放在寄存器的10-17号地址，其他参数被放在栈中；实现RISC的操作系统的内存中栈从高往低，堆从低往高安排分配；全局变量可以专门在寄存器中存放一指针gp来在内存中找到它们去调用；在程序运行中要调用函数必须把自己的临时变量都存到栈中去，之后把它们调回来，被调用函数同理。
